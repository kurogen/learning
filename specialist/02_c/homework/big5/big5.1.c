...
//#define MIN(u,v)																// * альтернативное решение Min/Max через макрос - НЕ ДОБРАЛСЯ!!!
...
int main()
{
	...
	int tmp_p;																	// Переменная, содержащая значение предыдущего произвольного числа (tmp_a)
	int p = 0;																	// Количество повторяющихся чисел (p)
		...
		sum += a;																// Суммирование введенных чисел в цикле (sum)
			...
			N ++;																// Счетчик количества введенных чисел равных заданному <<постоянному>> (N)
				...
				//(a) <= (tmp_a) ? (min=a) : (max=a);							// * альтернативное решение Min/Max через тернарный оператор - НЕ РАБОТАЕТ ПРАВИЛЬНО!!!
																				// * как указать <= и >= одновременно (79:12)?
				//MIN(min,max);													// * альтернативное решение Min/Max через макрос - НЕ ДОБРАЛСЯ!!!
					...
					p ++;
						...
						if (p == tmp_p)											// * сравнение повторяющегося числа - НЕ РАБОТАЕТ ПРАВИЛЬНО!!!
																				// * какое условие необходимо для правильной работы участков постоянства из трех и более чисел (88:14)? 
