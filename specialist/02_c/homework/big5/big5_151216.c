/*
	...
	5) Найти кол-во участков постоянства.
...
int main()
{
	...
	int p = 0;																	// Индикатор "0||1" повторяющегося числа (p)
	int tmp_p;																	// Индикатор "0||1" предыдущего повторяющегося числа (tmp_p)
	...
	for(i = 1; i < I; i++)
		...
		sum += a;																// Суммирование введенных чисел в цикле (sum)
			...
			N ++;																// Счетчик количества введенных чисел равных заданному <<постоянному>> (N)
				...
				(a) < (tmp_a) ? (min=a) : (max=a);								// Альтернативное решение Min/Max через тернарный оператор - НЕ РАБОТАЕТ ПРАВИЛЬНО!!!
				(a) > (tmp_a) ? (max=a) : (min=a);
				(a) == (tmp_a) ? (max=a) : (min=a);
				...
				MIN(min,max);													// *** альтернативное решение Min/Max через макрос - НЕ ПОНЯЛ!!!
					...
					p = 1;														// Значения индикатора "0||1" повторяющегося числа

						if (p == 1 && tmp_p != p)								// Разность значений индикаторов "0||1" переменных повторяющегося и предыдущего повторяющегося числа
						T ++;													// Счетчик количества участков постоянства (P)

							tmp_a = a;											// Сохранение значения в переменную предыдущего произвольного числа (tmp_a)
							tmp_p = p;											// Сохранение значения индикатора "0||1" в переменную предыдущего повторяющегося числа (tmp_p)
							p = 0;												// Сброс значения индикатора текущего числа в память (tmp_p)
		}
